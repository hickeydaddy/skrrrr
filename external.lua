...
function Module.BuildAllUI(Rayfield, Window, me, folderName, sendRequest, isVersionSafe, SAFE_PLACE_VERSION, currentVersion, UI_THEMES, selectedDisplayTheme)
    ...
    AboutTab:CreateParagraph({Title = "<font size=\"16\"><b>[*] Update: 5/27/2026</b></font>", Content = "• Multi-select Auto Quirk Roll implementation added with synchronous handshake.\n• Gamepass Unlocker added to Exploit section.\n• Extreme AFK FPS boost integrated to bypass entire Frame rendering natively.\n• Refactored Item utilities to cleanly house both Items and Crates."})
    ...
    local Toggle_Gamepass = MainTab:CreateToggle({Name = "Gamepass Unlocker", CurrentValue = false, Callback = function(V) _G.AutoGamepassActive = V end})
    ...
    MainTab:CreateDropdown({Name = "Select Item(s) to Use Max", Options = useItemList, CurrentOption = _G.SelectedUseItems, MultipleOptions = true, Callback = function(O) _G.SelectedUseItems = O end})
    ...
    _G.SelectedQuirks = {}
    MainTab:CreateDropdown({Name = "Choose Quirk(s) to auto roll", Options = quirkList, CurrentOption = _G.SelectedQuirks, MultipleOptions = true, Callback = function(O) _G.SelectedQuirks = O end})
